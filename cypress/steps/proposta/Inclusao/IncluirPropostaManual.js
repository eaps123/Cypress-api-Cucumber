import BuilderIncluirPropostaManual, {
    parseParametersTable,
    submitPropostaManual,
    mergeDataTableIntoPayload,
    preparePayloadForSubmit
} from "../../../support/Builders/proposta/Builder-IncluirPropostaManual";
import BuilderUtils, {
...
Given("que eu preencha o payload dto da Proposta Manual:", () => {
    PropostaPayload = {}
    PropostaPayload.parametros = {}
    cy.wrap({}).as('requestBody')
    cy.fixture('proposta/IncluirPropostaManual.json').then((dados) => {
        PropostaPayload = { ...dados }
        cy.log('[IncluirPropostaManual] payload.dto:', PropostaPayload.dto)
        cy.wrap(PropostaPayload).as('requestBody')
    })
})

And("defina parametros Proposta Manual:", (dataTable) => {
    const parametros = parseParametersTable(dataTable)
    PropostaPayload = PropostaPayload || {}
    PropostaPayload.parametros = parametros
    cy.log('[IncluirPropostaManual] parametros:', parametros)
    cy.wrap(PropostaPayload).as('requestBody')
})

And("remova os campos obrigatórios do DTO Proposta Manual {string}", (campoAusente) => {
...
And("que eu valide o metodo de pagamento Proposta Manual:", (dataTable) => {
    return cy.get('@requestBody').then((rb) => {
        PropostaPayload = PropostaPayload || {}
...
When("eu envio uma requisição POST para Proposta Manual", (dataTable) => {
    return cy.get('@requestBody').then((rb) => {
        const base = rb && rb.dto ? rb : { dto: rb || {} }
        const payload = preparePayloadForSubmit(base, dataTable)
        PropostaPayload = payload
        cy.wrap(JSON.parse(JSON.stringify(PropostaPayload))).as('requestBody')
        return submitPropostaManual(payload)
    })
})

Then("recebo status Proposta Manual {int}", (expectedStatus, dataTable) => {
