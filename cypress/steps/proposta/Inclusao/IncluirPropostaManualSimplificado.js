import BuilderIncluirPropostaManualSimplificado, {
    parseParametersTable,
    submitPropostaManualSimplificado,
    mergeDataTableIntoPayload,
    preparePayloadForSubmit
} from "../../../support/Builders/proposta/Builder-IncluirPropostaManualSimplificado";
import BuilderUtils, {
...
Given("que eu preencha o payload dto da proposta com ManualSimplificado:", () => {
    PropostaPayload = {}
    PropostaPayload.parametros = {}
    cy.wrap({}).as('requestBody')
    cy.fixture('proposta/IncluirPropostaManualSimplificado.json').then((dados) => {
        PropostaPayload = { ...dados }
        cy.log('[IncluirPropostaManualSimplificado] payload.dto:', PropostaPayload.dto)
        cy.wrap(PropostaPayload).as('requestBody')
    })
})

And("defina parametros ManualSimplificado:", (dataTable) => {
    const parametros = parseParametersTable(dataTable)
    PropostaPayload = PropostaPayload || {}
    PropostaPayload.parametros = parametros
    cy.log('[IncluirPropostaManualSimplificado] parametros:', parametros)
    cy.wrap(PropostaPayload).as('requestBody')
})

And("remova os campos obrigatórios do DTO ManualSimplificado {string}", (campoAusente) => {
...
And("que eu valide o metodo de pagamento ManualSimplificado:", (dataTable) => {
    return cy.get('@requestBody').then((rb) => {
        PropostaPayload = PropostaPayload || {}
...
When("eu envio uma requisição POST para proposta ManualSimplificado", (dataTable) => {
    return cy.get('@requestBody').then((rb) => {
        const base = rb && rb.dto ? rb : { dto: rb || {} }
        const payload = preparePayloadForSubmit(base, dataTable)
        PropostaPayload = payload
        cy.wrap(JSON.parse(JSON.stringify(PropostaPayload))).as('requestBody')
        return submitPropostaManualSimplificado(payload)
    })
})

Then("recebo status ManualSimplificado {int}", (expectedStatus, dataTable) => {
