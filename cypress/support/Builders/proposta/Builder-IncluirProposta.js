...
export const submitProposta = (payload = {}, { requests = propostaRequests, integration = DEFAULT_INTEGRATION, setEnv = setPropostaEnv } = {}) => {
    ...
    if (hasCodDto) {
        ...
        cy.log(`[IncluirProposta] CodigoOperacao gerado: ${novoCodigo}`)
    } else {
        cy.log(`[IncluirProposta] CodigoOperacao preservado: ${normalized.dto.CodigoOperacao}`)
    }
    const body = { ...normalized, dto: normalized.dto }
    return requests.IncluirProposta(body, integration).then((resp) => {
        cy.log('[IncluirProposta] response:', resp)
        cy.wrap(resp).as('responseStatus')
        const safeName = (scenarioTitle && scenarioTitle.length) ? scenarioTitle : 'IncluirProposta'
        const propostaCodigo = resp && resp.body && resp.body.Codigo ? resp.body.Codigo : 'unknown'
        try {
            setEnv && setEnv('IncluirProposta', resp)
        } catch (e) {
...
export default BuilderIncluirProposta;