...
export const submitPropostaManualSAC = (payload = {}, { requests = propostaRequests, integration = DEFAULT_INTEGRATION, setEnv = setPropostaEnv } = {}) => {
    ...
    if (hasCodDto) {
        ...
        cy.log(`[IncluirPropostaManualSAC] CodigoOperacao gerado: ${novoCodigo}`)
    } else {
        cy.log(`[IncluirPropostaManualSAC] CodigoOperacao preservado: ${normalized.dto.CodigoOperacao}`)
    }
    const body = { ...normalized, dto: normalized.dto }
    return requests.IncluirPropostaManualSAC(body, integration).then((resp) => {
        cy.log('[IncluirPropostaManualSAC] response:', resp)
        cy.wrap(resp).as('responseStatus')
        const safeName = (scenarioTitle && scenarioTitle.length) ? scenarioTitle : 'IncluirPropostaManualSAC'
        const propostaCodigo = resp && resp.body && resp.body.Codigo ? resp.body.Codigo : 'unknown'
        try {
            setEnv && setEnv('IncluirPropostaManualSAC', resp)
        } catch (e) {
...
const BuilderIncluirPropostaManualSAC = {
...
export default BuilderIncluirPropostaManualSAC;