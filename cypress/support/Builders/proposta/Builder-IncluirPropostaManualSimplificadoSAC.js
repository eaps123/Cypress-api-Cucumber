...
export const submitPropostaManualSimplificadoSAC = (payload = {}, { requests = propostaRequests, integration = DEFAULT_INTEGRATION, setEnv = setPropostaEnv } = {}) => {
    ...
    if (hasCodDto) {
        ...
        cy.log(`[IncluirPropostaManualSimplificadoSAC] CodigoOperacao gerado: ${novoCodigo}`)
    } else {
        cy.log(`[IncluirPropostaManualSimplificadoSAC] CodigoOperacao preservado: ${normalized.dto.CodigoOperacao}`)
    }
    const body = { ...normalized, dto: normalized.dto }
    return requests.IncluirPropostaManualSimplificadoSAC(body, integration).then((resp) => {
        cy.log('[IncluirPropostaManualSimplificadoSAC] response:', resp)
        cy.wrap(resp).as('responseStatus')
        const safeName = (scenarioTitle && scenarioTitle.length) ? scenarioTitle : 'IncluirPropostaManualSimplificadoSAC'
        const propostaCodigo = resp && resp.body && resp.body.Codigo ? resp.body.Codigo : 'unknown'
        try {
            setEnv && setEnv('IncluirPropostaManualSimplificadoSAC', resp)
        } catch (e) {
...
const BuilderIncluirPropostaManualSimplificadaSAC = {
...
export default BuilderIncluirPropostaManualSimplificadaSAC;