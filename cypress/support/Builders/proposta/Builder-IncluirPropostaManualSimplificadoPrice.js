...
export const submitPropostaManualSimplificadoPrice = (payload = {}, { requests = propostaRequests, integration = DEFAULT_INTEGRATION, setEnv = setPropostaEnv } = {}) => {
    ...
    if (hasCodDto) {
        ...
        cy.log(`[IncluirPropostaManualSimplificadoPrice] CodigoOperacao gerado: ${novoCodigo}`)
    } else {
        cy.log(`[IncluirPropostaManualSimplificadoPrice] CodigoOperacao preservado: ${normalized.dto.CodigoOperacao}`)
    }
    const body = { ...normalized, dto: normalized.dto }
    return requests.IncluirPropostaManualSimplificadoPrice(body, integration).then((resp) => {
        cy.log('[IncluirPropostaManualSimplificadoPrice] response:', resp)
        cy.wrap(resp).as('responseStatus')
        const safeName = (scenarioTitle && scenarioTitle.length) ? scenarioTitle : 'IncluirPropostaManualSimplificadoPrice'
        const propostaCodigo = resp && resp.body && resp.body.Codigo ? resp.body.Codigo : 'unknown'
        try {
            setEnv && setEnv('IncluirPropostaManualSimplificadoPrice', resp)
        } catch (e) {
...
const BuilderIncluirPropostaManualSimplificadoPrice = {
...
export default BuilderIncluirPropostaManualSimplificadoPrice;