...
export const submitPropostaManualSimplificado = (payload = {}, { requests = propostaRequests, integration = DEFAULT_INTEGRATION, setEnv = setPropostaEnv } = {}) => {
    ...
    if (hasCodDto) {
        ...
        cy.log(`[IncluirPropostaManualSimplificado] CodigoOperacao gerado: ${novoCodigo}`)
    } else {
        cy.log(`[IncluirPropostaManualSimplificado] CodigoOperacao preservado: ${normalized.dto.CodigoOperacao}`)
    }
    const body = { ...normalized, dto: normalized.dto }
    return requests.IncluirPropostaManualSimplificado(body, integration).then((resp) => {
        cy.log('[IncluirPropostaManualSimplificado] response:', resp)
        cy.wrap(resp).as('responseStatus')
        const safeName = (scenarioTitle && scenarioTitle.length) ? scenarioTitle : 'IncluirPropostaManualSimplificado'
        const propostaCodigo = resp && resp.body && resp.body.Codigo ? resp.body.Codigo : 'unknown'
        try {
            setEnv && setEnv('IncluirPropostaManualSimplificado', resp)
        } catch (e) {
...
const BuilderIncluirPropostaManualSimplificado = {
...
export default BuilderIncluirPropostaManualSimplificado;